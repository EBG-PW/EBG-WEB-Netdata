...
const elementSchema = Joi.object({
    ...
    hostname: Joi.fullysanitizedString().max(32).required(),
    ...
    value: Joi.safeNumber().safe().required(),
...
const parseNetdata = async (netdata_json) => {
    ...
    // Filter out elements with invalid number values
    const safeData = netdata_json.filter(element =>
        typeof element.value !== 'number' ||
        (element.value >= Number.MIN_SAFE_INTEGER && element.value <= Number.MAX_SAFE_INTEGER)
    );

    // Validate each element in the array against the schema
    for (const element of safeData) {
    ...
    const hostname = safeData[0].hostname;

    let filtered = safeData.filter((element) => {
    ...
    Object.keys(result.metrics).forEach(key => {
        // Calculate average for metrics that have 'avg' operation
        ...
        // Remove sum for metrics that have 'sum' operation
        if (result.metrics[key].count === 0) {
            result.metrics[key].value = result.metrics[key].sum;
            delete result.metrics[key].count;
            delete result.metrics[key].sum;
        }
    });
