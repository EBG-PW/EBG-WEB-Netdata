...
const { GetNetdataMonitor } = require('@lib/redis');
...
router.post('/put', /* limiter(1) ,*/ async (req, res) => {
    ...
    const chart_fields = NetdataCharts.listCharts(monitor_config.charts);
    console.log(monitor_config, chart_fields);
    chart_fields.forEach(async (chart) => {
        console.log(`Saving chart ${chart} for ${parsedData.hostname}`, NetdataCharts.getChartFields(chart));
    });
